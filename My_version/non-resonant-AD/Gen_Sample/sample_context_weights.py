...
parser.add_argument("-i","--indir",help="working folder",default="../../working_dir/")
...
def main():
    ...
    for p in per:
        ...
        #Load Model
        if os.path.isfile(f"{model_path}/context_weight_{p}.pt"):
            print("Loading model")
            NN_Context_Weights = torch.load(f"{model_path}/context_weight_{p}.pt")
        ...
        np.savez(f"{samples_path}/context_weight_CR_samples_{p}.npz", target_cr=data_cr_test, mc_cr=mc_cr_test, w_cr=np.nan_to_num(w_cr, copy=False, nan=0.0, posinf=0.0, neginf=0.0))
        
        # SR Samples weights
        mc_events_sr = mc_events_sr[:,:n_context]
        w_sr = NN_Context_Weights.evaluation(mc_events_sr)
        w_sr = (w_sr/(1.-w_sr)).flatten()
        np.savez(f"{samples_path}/context_weight_SR_samples_{p}.npz", mc_samples=mc_events_sr, w_sr=np.nan_to_num(w_sr, copy=False, nan=0.0, posinf=0.0, neginf=0.0))
