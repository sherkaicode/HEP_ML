...
parser.add_argument("-s","--signal",default=0,help="signal fraction")
...
#         print()
    ...
    if args.reweight:
        reweight_events = np.load(f"{samples_dir}/reweight_SR_samples_{args.signal}.npz")
        ...
        run_eval(set_1, set_2, w_1 = w_1, code=f"reweight_{args.signal}_sr", save_dir=eval_dir, classifier_params=params, device=device, run_test=True, test_B=test_bkg, test_S=test_sig, crop_weights=True)
        print()
        
    if args.generate:
        generate_events = np.load(f"{samples_dir}/generate_SR_samples_{args.signal}.npz")
        context_weights = np.load(f"{samples_dir}/context_weight_SR_samples_{args.signal}.npz")
        data_events = np.load(path_to_data)    
        set_1 = generate_events["samples"]
        w_1 =  context_weights["w_sr"]
        set_2 = data_events["data_events_sr"][:,n_context:]
        run_eval(set_1, set_2, w_1 = w_1, code=f"generate_{args.signal}_sr", save_dir=eval_dir, classifier_params=params, device=device, run_test=True, test_B=test_bkg, test_S=test_sig, crop_weights=True)
        print()
        
    if args.morph:
        morph_events = np.load(f"{samples_dir}/morph_SR_samples_{args.signal}.npz")
        context_weights = np.load(f"{samples_dir}/context_weight_SR_samples_{args.signal}.npz")
        data_events = np.load(path_to_data)    
        set_1 = morph_events["samples"]
        w_1 =  context_weights["w_sr"]
        set_2 = data_events["data_events_sr"][:,n_context:]
        run_eval(set_1, set_2, w_1 = w_1, code=f"morph_{args.signal}_sr", save_dir=eval_dir, classifier_params=params, device=device, run_test=True, test_B=test_bkg, test_S=test_sig, crop_weights=True)
        print()
